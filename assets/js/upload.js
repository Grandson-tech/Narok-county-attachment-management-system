...
async function loadDocuments() {
  ...
  const documentsWithLinks = await Promise.all(documents.map(async (doc) => {
    const { data, error: linkError } = await supabase.storage.from('student-documents').createSignedUrl(doc.file_path, 60 * 10);
    return { ...doc, fileUrl: linkError ? null : data.signedUrl };
  }));
  list.innerHTML = documentsWithLinks.length ? documentsWithLinks.map((doc) => `<article class="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-semibold">Acceptance letter</p><p class="mt-1 text-sm text-slate-500">${doc.file_name}</p>${doc.reviewer_notes ? `<p class="mt-1 text-sm text-slate-600">HR note: ${doc.reviewer_notes}</p>` : ''}${doc.fileUrl ? `<a href="${doc.fileUrl}" target="_blank" rel="noopener noreferrer" class="mt-2 inline-flex text-sm font-bold text-county-green hover:underline">View uploaded file <span aria-hidden="true">↗</span></a>` : '<p class="mt-2 text-xs text-slate-500">File preview is temporarily unavailable.</p>'}</div><div class="flex items-center gap-3"><span class="rounded-full px-3 py-1 text-xs font-bold ${doc.review_status === 'approved' ? 'bg-emerald-100 text-emerald-800' : doc.review_status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}">${label(doc.review_status)}</span>${doc.review_status === 'pending' ? `<button data-delete="${doc.id}" data-path="${doc.file_path}" class="text-sm font-bold text-red-700 hover:underline">Remove</button>` : ''}</div></article>`).join('') : '<p class="p-5 text-sm text-slate-500">No acceptance letter has been sent.</p>';
